...
# Load gdb layer
```{r load_lyr}
load_mc_gdb <- function(dir, gdb, lyr, tbl, verbose) {
  # source(here::here("functions.R"))
  source(here::here("scripts/shp_mc.R"))
  sf::st_layers(gdb)
  ...
  if(verbose) {
    ...
    if(dbExistsTable(con, tbl)) {
      cat(sprintf('Success: %s -> %s', lyr, tbl))
    } else {
      cat(sprintf('Failed to load: %s -> %s', lyr, tbl))
    }
  }
...
df = pd.DataFrame(lyr_list)
```

# Example Dataset Load
```{r example-dataset, eval=F}
...
for (i in 1:nrow(datasets_to_load)) {
  ...
  lyr <- datasets_to_load[i, 3]
  tbl <- datasets_to_load[i, 4]
}
