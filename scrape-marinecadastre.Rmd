...
csv_mc_gh    <- "/share/github/mhk-env_shiny-apps/data/datasets_marinecadastre.gov.csv"
...
get_page = function(url, js='scrape.js', html='scraped.html'){
  ...
  # cd /share/github/mhk-env_shiny-apps; /usr/local/bin/phantomjs scrape.js
...
datasets_csv <- glue::glue("https://docs.google.com/spreadsheets/d/{datasets_gid}/gviz/tq?tqx=out:csv&sheet=modals")

d_shps  <- read_csv(datasets_csv) %>% 
  ...
  unnest(shp_path) %>% 
  mutate(
    shp_fname   = basename(shp_path),
    shp_size_mb = dir_size_mb / shps_n,
    shp_tbl     = map2_chr(title, shp_fname, titleshp2tbl),
    shp_tbl_duplicated = duplicated(shp_tbl)) %>% 
  arrange(desc(shp_size_mb))
  #arrange(shp_tbl)

stopifnot(sum(d_shps$shp_tbl_duplicated) == 0)

d_shps %>%
  select(shp_tbl, shp_tbl_duplicated, title, shp_fname, priority, dataset_code, shps_n, dir_size_mb, shp_size_mb,  shp_path) %>%
  View()
