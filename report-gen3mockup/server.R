shinyServer(function(input, output, session) {
  ...
  output$tblReports <- DT::renderDataTable({
    d <- tribble(
      ~Title, ~Literature, ~Spatial, ~Management, ~Edit, ~Delete,
      "Birds and Tidal Technology", T, F, T, "∆", "-",
      "Marine Mammals in Cobscook Bay", F, T, T, "∆", "-",
      "Testing Reporting", T, T, T, "∆", "-",)
    d
  })
